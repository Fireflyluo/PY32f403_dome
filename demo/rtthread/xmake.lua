...
target("rtthread")
    ...
    on_load(function(target)
        local rtt = import("os.rtt", {rootdir = os.scriptdir()})
        rtt.config_add(target)
        rtt.kernel_add(target)
        rtt.components_auto_add(target)
    end)
...
task("menuconfig")
    ...
    on_run(function()
        local rtt_root = "D:/MCU/6.OS/RTT/rt-thread-v5.1.0"
        local kconfig_file = path.join(os.scriptdir(), "os/Kconfig")
        os.setenv("RTT_ROOT", rtt_root)
        os.setenv("BSP_ROOT", os.scriptdir())
        os.setenv("PKGS_ROOT", path.join(os.scriptdir(), "packages"))
        os.execv("python", {
            path.join(rtt_root, "tools/menuconfig.py"),
