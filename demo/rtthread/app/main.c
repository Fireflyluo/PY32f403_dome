...
#define WS2812_DMA_CHANNEL DMA2_Channel5
#define WS2812_DMA_IRQ DMA2_Channel4_5_IRQn
...
static void ws2812_fill_buffer(void)
{
    ...
    for (i = 0; i < WS2812_NUM; i++)
    {
        ...
        for (b = 0; b < 24; b++)
        {
            ws2812_buf[bit_index++] = (grb & 0x800000U) ? 120U : 60U;
            grb <<= 1;
        }
    }
...
static volatile int breath_stop = 0;

static uint32_t _color_dim(uint32_t c, int b)
{
...
static void ws2812_breath_entry(void *param)
{
    uint32_t base = *(uint32_t *)param;
    int i, b, dir;
    rt_tick_t start, elapsed;

    rt_kprintf("WS2812 breath start, color=#%06lX\n", (unsigned long)base);
    breath_stop = 0;
    b = 0;
    dir = 1;
    ...
    while (!breath_stop)
    {
        start = rt_tick_get();
        for (i = 0; i < WS2812_NUM; i++)
            ws2812_color[i] = _color_dim(base, b);
        ws2812_show();

        elapsed = rt_tick_get() - start;
        if (elapsed < 10)
            rt_thread_mdelay(10 - elapsed);

        b += dir * 2;
        if (b >= 255) { b = 255; dir = -1; }
        if (b <= 0)   { b = 0;   dir = 1; }
    }
    rt_kprintf("WS2812 breath stopped\n");
}

static int cmd_ws2812_breath(int argc, char **argv)
{
    static uint32_t param;
    uint32_t v = 0x00FF00;

    if (breath_thread && (breath_thread->stat & RT_THREAD_STAT_MASK) != RT_THREAD_CLOSE)
    {
        breath_stop = 1;
        rt_thread_mdelay(50);
        breath_thread = RT_NULL;
        rt_kprintf("Breath stop requested\n");
        return RT_EOK;
    }

    if (argc >= 2) v = (uint32_t)strtoul(argv[1], RT_NULL, 16) & 0xFFFFFFU;
    param = v;

    breath_thread = rt_thread_create("ws_br", ws2812_breath_entry, &param, 1024, 22, 10);
    if (!breath_thread) { rt_kprintf("Failed\n"); return -RT_ENOMEM; }
    rt_thread_startup(breath_thread);
    rt_kprintf("Breath started (再输一次停止)\n");
    return RT_EOK;
}
MSH_CMD_EXPORT(cmd_ws2812_breath, WS2812 breathing effect);
