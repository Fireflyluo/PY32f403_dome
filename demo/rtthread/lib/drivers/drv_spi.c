...
static struct py32_spi_config spi_config[] =
{
#ifdef BSP_USING_SPI1
    SPI1_BUS_CONFIG,
#endif
...
static struct py32_spi spi_bus_obj[sizeof(spi_config) / sizeof(spi_config[0])] = {0};

static rt_err_t py32_spi_init(struct py32_spi *spi_drv, struct rt_spi_configuration *cfg)
{
...
#ifdef SPI_SLAVE_FAST_DISABLE
    spi_handle->Init.SlaveFastMode = SPI_SLAVE_FAST_DISABLE;
#endif
    ...
    {
        py32_dma_apply_remap(spi_drv->config->dma_rx);
    ...
    {
        py32_dma_apply_remap(spi_drv->config->dma_tx);
...
static rt_ssize_t spixfer(struct rt_spi_device *device, struct rt_spi_message *message)
{
    ...
    struct py32_spi *spi_drv =  rt_container_of(device->bus, struct py32_spi, spi_bus);
    SPI_HandleTypeDef *spi_handle = &spi_drv->handle;
    ...
    while (message_length)
    {
        ...
        {
        if ((spi_drv->spi_dma_flag & SPI_USING_TX_DMA_FLAG) && (send_length >= DMA_TRANS_MIN_LEN) && send_buf != RT_NULL)
            {
                state = HAL_SPI_Transmit_DMA(spi_handle, (uint8_t *)p_txrx_buffer, send_length);
            }
            else
                              ...
                              struct rt_spi_configuration *configuration)
{
    ...
    return py32_spi_init(spi_drv, configuration);
}
...
{
    struct py32_spi *spi_drv =  rt_container_of(hspi, struct py32_spi, handle);
