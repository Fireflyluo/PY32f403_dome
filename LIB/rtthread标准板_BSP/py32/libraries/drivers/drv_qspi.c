...
struct py32_qspi_bus _py32_qspi_bus;

static int py32_qspi_init(struct rt_qspi_device *device, struct rt_qspi_configuration *qspi_cfg)
{
    int result = RT_EOK;
    unsigned int i = 1;
    ...
    struct py32_qspi_bus *qspi_bus = device->parent.bus->parent.user_data;
...
static void qspi_send_cmd(struct py32_qspi_bus *qspi_bus, struct rt_qspi_message *message)
{
...
static rt_ssize_t qspixfer(struct rt_spi_device *device, struct rt_spi_message *message)
{
    ...
    struct py32_qspi_bus *qspi_bus = device->bus->parent.user_data;
...
static rt_err_t qspi_configure(struct rt_spi_device *device, struct rt_spi_configuration *configuration)
{
    ...
    return py32_qspi_init(qspi_device, &qspi_device->config);
}

static const struct rt_spi_ops py32_qspi_ops =
{
    .configure = qspi_configure,
    .xfer = qspixfer,
};

static int py32_qspi_register_bus(struct py32_qspi_bus *qspi_bus, const char *name)
{
    RT_ASSERT(qspi_bus != RT_NULL);
    RT_ASSERT(name != RT_NULL);

    _qspi_bus1.parent.user_data = qspi_bus;
    return rt_qspi_bus_register(&_qspi_bus1, name, &py32_qspi_ops);
}
...
void QSPI_IRQHandler(void)
{
    ...
    HAL_QSPI_IRQHandler(&_py32_qspi_bus.QSPI_Handler);
...
void QSPI_DMA_IRQHandler(void)
{
    ...
    HAL_DMA_IRQHandler(&_py32_qspi_bus.hdma_quadspi);
...
{
    return py32_qspi_register_bus(&_py32_qspi_bus, "qspi1");
}
