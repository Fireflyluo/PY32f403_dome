...
static struct rt_py32_eth py32_eth_device;
...
static rt_err_t rt_py32_eth_init(rt_device_t dev)
{
    ...
    EthHandle.Init.MACAddr = (rt_uint8_t *)&py32_eth_device.dev_addr[0];
...
static rt_err_t rt_py32_eth_open(rt_device_t dev, rt_uint16_t oflag)
{
    LOG_D("emac open");
    return RT_EOK;
}

static rt_err_t rt_py32_eth_close(rt_device_t dev)
{
    LOG_D("emac close");
    return RT_EOK;
}

static rt_ssize_t rt_py32_eth_read(rt_device_t dev, rt_off_t pos, void *buffer, rt_size_t size)
{
    LOG_D("emac read");
...
static rt_ssize_t rt_py32_eth_write(rt_device_t dev, rt_off_t pos, const void *buffer, rt_size_t size)
{
    LOG_D("emac write");
    rt_set_errno(-RT_ENOSYS);
    return 0;
...
static rt_err_t rt_py32_eth_control(rt_device_t dev, int cmd, void *args)
{
    switch (cmd)
    {
    case NIOCTL_GADDR:
        ...
        if (args)
        {
            SMEMCPY(args, py32_eth_device.dev_addr, 6);
        }
        else
        {
            return -RT_ERROR;
...
/* transmit data*/
rt_err_t rt_py32_eth_tx(rt_device_t dev, struct pbuf *p)
{
...
/* receive data*/
struct pbuf *rt_py32_eth_rx(rt_device_t dev)
{
...
{
    rt_err_t result;
    result = eth_device_ready(&(py32_eth_device.parent));
    if (result != RT_EOK)
    {
...
static void phy_linkchange()
{
    ...
    if (phy_speed != phy_speed_new)
    {
        ...
        if (phy_speed & PHY_LINK)
        {
            ...
            {
                LOG_D("100Mbps");
                py32_eth_device.ETH_Speed = ETH_SPEED_100M;
            }
            else
            {
                py32_eth_device.ETH_Speed = ETH_SPEED_10M;
                LOG_D("10Mbps");
            }
            ...
            {
                LOG_D("full-duplex");
                py32_eth_device.ETH_Mode = ETH_MODE_FULLDUPLEX;
            }
            else
            {
                LOG_D("half-duplex");
                py32_eth_device.ETH_Mode = ETH_MODE_HALFDUPLEX;
            }

            /* send link up. */
            eth_device_linkchange(&py32_eth_device.parent, RT_TRUE);
        }
        else
        {
            LOG_I("link down");
            eth_device_linkchange(&py32_eth_device.parent, RT_FALSE);
        }
    }
}
...
static void phy_monitor_thread_entry(void *parameter)
{
    ...
    py32_eth_device.poll_link_timer = rt_timer_create("phylnk", (void (*)(void*))phy_linkchange,
                                        NULL, RT_TICK_PER_SECOND, RT_TIMER_FLAG_PERIODIC);
    if (!py32_eth_device.poll_link_timer || rt_timer_start(py32_eth_device.poll_link_timer) != RT_EOK)
    {
...
static int rt_hw_py32_eth_init(void)
{
    ...
    py32_eth_device.dev_addr[0] = 0x00;
    py32_eth_device.dev_addr[1] = 0x80;
    py32_eth_device.dev_addr[2] = 0xE1;
    /* generate MAC addr from 96bit unique ID (only for test). */
    py32_eth_device.dev_addr[3] = *(rt_uint8_t *)(UID_BASE + 4);
    py32_eth_device.dev_addr[4] = *(rt_uint8_t *)(UID_BASE + 2);
    py32_eth_device.dev_addr[5] = *(rt_uint8_t *)(UID_BASE + 0);

    py32_eth_device.parent.parent.init       = rt_py32_eth_init;
    py32_eth_device.parent.parent.open       = rt_py32_eth_open;
    py32_eth_device.parent.parent.close      = rt_py32_eth_close;
    py32_eth_device.parent.parent.read       = rt_py32_eth_read;
    py32_eth_device.parent.parent.write      = rt_py32_eth_write;
    py32_eth_device.parent.parent.control    = rt_py32_eth_control;
    py32_eth_device.parent.parent.user_data  = RT_NULL;

    py32_eth_device.parent.eth_rx     = rt_py32_eth_rx;
    py32_eth_device.parent.eth_tx     = rt_py32_eth_tx;

    /* register eth device */
    state = eth_device_init(&(py32_eth_device.parent), "e0");
    if (RT_EOK == state)
    {
...
INIT_DEVICE_EXPORT(rt_hw_py32_eth_init);
