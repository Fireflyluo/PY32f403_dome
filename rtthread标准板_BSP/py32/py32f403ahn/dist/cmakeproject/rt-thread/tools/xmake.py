...
class XmakeProject:
    ...
    def generate_xmake_file(self):
        if os.getenv('RTT_ROOT'):
            ...
            # Try to find RTT_ROOT by looking for tools/xmake.lua
            cwd = os.getcwd()
            RTT_ROOT = None
            
            # First, check if rt-thread directory exists in current dir (for dist projects)
            if os.path.exists(os.path.join(cwd, 'rt-thread', 'tools', 'xmake.lua')):
                RTT_ROOT = os.path.join(cwd, 'rt-thread')
            else:
                # Try different parent levels (for regular BSP projects)
                for parent_level in ['../../../..', '../../..', '../../../']:
                    try_path = os.path.normpath(os.path.join(cwd, parent_level))
                    if os.path.exists(os.path.join(try_path, 'tools', 'xmake.lua')):
                        RTT_ROOT = try_path
                        break
                if RTT_ROOT is None:
                    RTT_ROOT = os.path.normpath(os.path.join(cwd, '../../..'))
