...
static rt_err_t py32_control(struct rt_serial_device *serial, int cmd, void *arg)
{
    ...
    case RT_DEVICE_CTRL_CONFIG:
        if (ctrl_arg & (RT_DEVICE_FLAG_DMA_RX | RT_DEVICE_FLAG_DMA_TX))
        {
            ...
            py32_control(serial, RT_DEVICE_CTRL_SET_INT, (void *)ctrl_arg);
        break;
...
static int py32_getc(struct rt_serial_device *serial)
{
    ...
    if (__HAL_UART_GET_FLAG(&(uart->handle), UART_FLAG_RXNE) != RESET)
        ch = UART_GET_RDR(&uart->handle, py32_uart_get_mask(uart->handle.Init.WordLength, uart->handle.Init.Parity));
    return ch;
}

static rt_ssize_t py32_transmit(struct rt_serial_device     *serial,
                                       rt_uint8_t           *buf,
                                       rt_size_t             size,
                                       rt_uint32_t           tx_flag)
{
    ...
    py32_control(serial, RT_DEVICE_CTRL_SET_INT, (void *)tx_flag);
...
static void uart_isr(struct rt_serial_device *serial)
{
            ...
            (__HAL_UART_GET_IT_SOURCE(&(uart->handle), UART_IT_RXNE) != RESET))
    {
        ...
        rt_ringbuffer_putchar(&(rx_fifo->rb), UART_GET_RDR(&uart->handle, py32_uart_get_mask(uart->handle.Init.WordLength, uart->handle.Init.Parity)));
...
static void py32_uart_get_config(void)
{
...
static const struct rt_uart_ops py32_uart_ops =
{
    ...
    .putc = py32_putc,
    .getc = py32_getc,
    .transmit = py32_transmit
};

int rt_hw_usart_init(void)
{
    rt_err_t result = 0;
    rt_size_t obj_num = sizeof(uart_obj) / sizeof(struct py32_uart);

    py32_uart_get_config();
