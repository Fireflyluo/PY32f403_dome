...
static struct py32_soft_spi_config soft_spi_config[] =
{
...
static struct py32_soft_spi spi_obj[sizeof(soft_spi_config) / sizeof(soft_spi_config[0])];
...
static void py32_spi_gpio_init(struct py32_soft_spi *spi)
{
    struct py32_soft_spi_config *cfg = (struct py32_soft_spi_config *)spi->cfg;
...
void py32_tog_sclk(void *data)
{
    struct py32_soft_spi_config* cfg = (struct py32_soft_spi_config*)data;
...
void py32_set_sclk(void *data, rt_int32_t state)
{
...
void py32_set_mosi(void *data, rt_int32_t state)
{
    struct py32_soft_spi_config* cfg = (struct py32_soft_spi_config*)data;
    if (state)
    {
...
void py32_set_miso(void *data, rt_int32_t state)
{
    struct py32_soft_spi_config* cfg = (struct py32_soft_spi_config*)data;
    if (state)
    {
...
rt_int32_t py32_get_sclk(void *data)
{
    struct py32_soft_spi_config* cfg = (struct py32_soft_spi_config*)data;
    return rt_pin_read(cfg->sck);
}

rt_int32_t py32_get_mosi(void *data)
{
    struct py32_soft_spi_config* cfg = (struct py32_soft_spi_config*)data;
    return rt_pin_read(cfg->mosi);
}

rt_int32_t py32_get_miso(void *data)
{
    struct py32_soft_spi_config* cfg = (struct py32_soft_spi_config*)data;
    return rt_pin_read(cfg->miso);
}

void py32_dir_mosi(void *data, rt_int32_t state)
{
    struct py32_soft_spi_config* cfg = (struct py32_soft_spi_config*)data;
    if (state)
    {
...
void py32_dir_miso(void *data, rt_int32_t state)
{
    struct py32_soft_spi_config* cfg = (struct py32_soft_spi_config*)data;
    if (state)
    {
...
static void py32_udelay(rt_uint32_t us)
{
    rt_uint32_t ticks;
...
static void py32_pin_init(void)
{
    rt_size_t obj_num = sizeof(spi_obj) / sizeof(struct py32_soft_spi);

    for(rt_size_t i; i < obj_num; i++)
    {
        py32_spi_gpio_init(&spi_obj[i]);
    }
}

static struct rt_spi_bit_ops py32_soft_spi_ops =
    {
        .data = RT_NULL,
        .pin_init = py32_pin_init,
        .tog_sclk = py32_tog_sclk,
        .set_sclk = py32_set_sclk,
        .set_mosi = py32_set_mosi,
        .set_miso = py32_set_miso,
        .get_sclk = py32_get_sclk,
        .get_mosi = py32_get_mosi,
        .get_miso = py32_get_miso,
        .dir_mosi = py32_dir_mosi,
        .dir_miso = py32_dir_miso,
        .udelay = py32_udelay,
        .delay_us = 1,
};
...
int rt_hw_softspi_init(void)
{
    ...
    {
        memcpy(&spi_obj[i].ops, &py32_soft_spi_ops, sizeof(struct rt_spi_bit_ops));
        spi_obj[i].ops.data = (void *)&soft_spi_config[i];
        spi_obj[i].spi.ops = &py32_soft_spi_ops;
