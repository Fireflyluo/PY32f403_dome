...
#define PIN_STPORT(pin)                                                        \
  ((GPIO_TypeDef *)(GPIOA_BASE + (0x400u * PIN_PORT(pin))))
#define PIN_STPIN(pin) ((uint16_t)(1u << PIN_NO(pin)))

#if defined(GPIOE)
#define __PY32_PORT_MAX 5u
#elif defined(GPIOD)
#define __PY32_PORT_MAX 4u
#elif defined(GPIOC)
#define __PY32_PORT_MAX 3u
#elif defined(GPIOB)
#define __PY32_PORT_MAX 2u
#elif defined(GPIOA)
#define __PY32_PORT_MAX 1u
#else
#define __PY32_PORT_MAX 0u
#error Unsupported PY32 GPIO peripheral.
#endif

#define PIN_STPORT_MAX __PY32_PORT_MAX
...
static void py32_pin_write(rt_device_t dev, rt_base_t pin, rt_uint8_t value) {
...
static rt_ssize_t py32_pin_read(rt_device_t dev, rt_base_t pin) {
  GPIO_TypeDef *gpio_port;
  uint16_t gpio_pin;
...
static void py32_pin_mode(rt_device_t dev, rt_base_t pin, rt_uint8_t mode) {
...
static rt_err_t py32_pin_dettach_irq(struct rt_device *device, rt_base_t pin) {
  rt_base_t level;
  rt_int32_t irqindex = -1;
...
static rt_err_t py32_pin_irq_enable(struct rt_device *device, rt_base_t pin,
                                    rt_uint8_t enabled) {
  ...
  if (enabled == PIN_IRQ_ENABLE) {
    ...
    } else if ((irqmap->pinbit >= GPIO_PIN_10) &&
               (irqmap->pinbit <= GPIO_PIN_15)) {
      if (!(pin_irq_enable_mask & (GPIO_PIN_10 | GPIO_PIN_11 | GPIO_PIN_12 |
...
static const struct rt_pin_ops _py32_pin_ops = {
    py32_pin_mode,       py32_pin_write,       py32_pin_read,
    py32_pin_attach_irq, py32_pin_dettach_irq, py32_pin_irq_enable,
    py32_pin_get,
};
...
int rt_hw_pin_init(void) {
  ...
  return rt_device_pin_register("pin", &_py32_pin_ops, RT_NULL);
}
